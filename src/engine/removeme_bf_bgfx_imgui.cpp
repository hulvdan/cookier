...
struct OcornutImguiContext {
  void render(ImDrawData* _drawData) {
    ...
    const bgfx::Caps* caps = bgfx::getCaps();
    {
      ...
      bx::mtxOrtho(
        ortho, x, x + width, y + height, y, 0.0f, 1000.0f, 0.0f, caps->homogeneousDepth
      );
    ...
    for (int32_t ii = 0, num = _drawData->CmdListsCount; ii < num; ++ii) {
           ...
           ++cmd)
      {
        if (cmd->UserCallback) {
          ...
          if (clipRect.x < dispWidth && clipRect.y < dispHeight && clipRect.z >= 0.0f
              && clipRect.w >= 0.0f)
          {
    ...
    int32_t  height;
    {
      ...
      for (uint32_t ii = 0; ii < BX_COUNTOF(s_fontRangeMerge); ++ii) {
        ...
        io.Fonts->AddFontFromMemoryTTF(
          (void*)frm.data, (int)frm.size, _fontSize - 3.0f, &config, frm.ranges
        );
      }
    }
...
);  // error C4505: '' : unreferenced local function has been removed
BX_PRAGMA_DIAGNOSTIC_IGNORED_CLANG_GCC("-Wunused-function"
);  // warning: 'int rect_width_compare(const void*, const void*)' defined but not used
