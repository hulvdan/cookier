...
enum {
    ...
    = LAY_ITEM_BOX_MODEL_MASK | (LAY_ITEM_LAYOUT_MASK & ~LAY_BREAK) | LAY_USERMASK
};

LAY_STATIC_INLINE lay_vec4
lay_vec4_xyzw(lay_scalar x, lay_scalar y, lay_scalar z, lay_scalar w) {
...
LAY_EXPORT void
lay_get_size_xy(lay_context* ctx, lay_id item, lay_scalar* x, lay_scalar* y);
...
LAY_EXPORT void
lay_set_size_xy(lay_context* ctx, lay_id item, lay_scalar width, lay_scalar height);
...
LAY_EXPORT void     lay_get_margins_ltrb(
        lay_context* ctx,
        lay_id       item,
        lay_scalar*  l,
        lay_scalar*  t,
...
LAY_EXPORT void lay_set_margins_ltrb(
    lay_context* ctx,
    lay_id       item,
    lay_scalar   l,
    lay_scalar   t,
...
static LAY_FORCE_INLINE lay_scalar
lay_calc_overlayed_size(lay_context* ctx, lay_id item, int dim) {
...
static LAY_FORCE_INLINE lay_scalar
lay_calc_stacked_size(lay_context* ctx, lay_id item, int dim) {
    const int                wdim      = dim + 2;
    lay_item_t* LAY_RESTRICT pitem     = lay_get_item(ctx, item);
    lay_scalar               need_size = 0;
    lay_id                   child     = pitem->first_child;
...
static LAY_FORCE_INLINE lay_scalar
lay_calc_wrapped_overlayed_size(lay_context* ctx, lay_id item, int dim) {
    const int                wdim       = dim + 2;
    lay_item_t* LAY_RESTRICT pitem      = lay_get_item(ctx, item);
    lay_scalar               need_size  = 0;
...
static LAY_FORCE_INLINE lay_scalar
lay_calc_wrapped_stacked_size(lay_context* ctx, lay_id item, int dim) {
    const int                wdim       = dim + 2;
    lay_item_t* LAY_RESTRICT pitem      = lay_get_item(ctx, item);
    lay_scalar               need_size  = 0;
    lay_scalar               need_size2 = 0;
...
lay_arrange_stacked(lay_context* ctx, lay_id item, int dim, bool wrap) {
    const int   wdim  = dim + 2;
    lay_item_t* pitem = lay_get_item(ctx, item);
...
static LAY_FORCE_INLINE void lay_arrange_overlay(lay_context* ctx, lay_id item, int dim) {
    ...
    while (child != LAY_INVALID_ID) {
        ...
        case LAY_RIGHT:
            child_rect[dim]
                += space - child_rect[2 + dim] - child_margins[dim] - child_margins[wdim];
            break;
        case LAY_HFILL:
            child_rect[2 + dim]
                = lay_scalar_max(0, space - child_rect[dim] - child_margins[wdim]);
            break;
        default:
            break;
        }
...
static LAY_FORCE_INLINE lay_scalar
lay_arrange_wrapped_overlay_squeezed(lay_context* ctx, lay_id item, int dim) {
    const int   wdim        = dim + 2;
    lay_item_t* pitem       = lay_get_item(ctx, item);
    ...
    while (child != LAY_INVALID_ID) {
        ...
        if (pchild->flags & LAY_BREAK) {
            lay_arrange_overlay_squeezed_range(
                ctx, dim, start_child, child, offset, need_size
            );
            offset += need_size;
            start_child = child;
            need_size   = 0;
    ...
    lay_arrange_overlay_squeezed_range(
        ctx, dim, start_child, LAY_INVALID_ID, offset, need_size
    );
    offset += need_size;
    return offset;
}
...
static void lay_arrange(lay_context* ctx, lay_id item, int dim) {
    ...
    case LAY_ROW:
        if ((flags & 1) == (uint32_t)dim) {
            ...
            const lay_vec4 rect = ctx->rects[item];
            lay_arrange_overlay_squeezed_range(
                ctx, dim, pitem->first_child, LAY_INVALID_ID, rect[dim], rect[2 + dim]
            );
        }
        break;
    default:
